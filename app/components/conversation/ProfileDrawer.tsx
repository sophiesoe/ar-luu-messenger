"use client";

import useOtherUser from "@/app/hooks/useOtherUsers";
import { Conversation, User } from "@prisma/client";
import { Fragment, useMemo, useState } from "react";
import { format } from "date-fns";
import { Transition, Dialog } from "@headlessui/react";
import { IoClose, IoTrash } from "react-icons/io5";
import Avatar from "@/app/commons/avatar/Avatar";
import ConfirmModal from "../ConfirmModal";
interface ProfileDrawerProps {
  data: Conversation & {
    users: User[];
  };
  isOpened: boolean;
  onCloseDrawer: () => void;
}

const ProfileDrawer: React.FC<ProfileDrawerProps> = ({
  data,
  isOpened,
  onCloseDrawer,
}) => {
  const otherUser = useOtherUser(data);
  const [confirmOpened, setConfirmOpened] = useState(false);
  const joinedDate = useMemo(() => {
    return format(new Date(otherUser.createdAt), "PP");
  }, [otherUser.createdAt]);

  const title = useMemo(() => {
    return data.name || otherUser.name;
  }, [data.name, otherUser.name]);

  const statusText = useMemo(() => {
    if (data.isGroup) {
      return `${data.users.length} members`;
    }
    return "Active now";
  }, [data]);

  return (
    <>
      <ConfirmModal
        isOpen={confirmOpened}
        onClose={() => setConfirmOpened(false)}
      />

      <Transition.Root show={isOpened} as={Fragment}>
        <Dialog as="div" className="relative z-50" onClose={onCloseDrawer}>
          <Transition.Child
            as={Fragment}
            enter="ease-out duration-500"
            enterFrom="opacity-0"
            enterTo="opacity-100"
            leave="ease-in duration-500"
            leaveFrom="opacity-100"
            leaveTo="opacity-0"
          >
            <div className="fixed inset-0 bg-black bg-opacity-40" />
          </Transition.Child>
          <div className="fixed inset-0 overflow-hidden">
            <div className="absolutes inset-0 overflow-hidden">
              <div className="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10">
                <Transition.Child
                  as={Fragment}
                  enter="transform transition ease-in-out duration-500"
                  enterFrom="translate-x-full"
                  enterTo="translate-x-0"
                  leave="transform transition ease-in-out duration-500"
                  leaveFrom="translate-x-0"
                  leaveTo="translate-x-full"
                >
                  <Dialog.Panel className="pointer-events-auto w-screen max-w-md">
                    <div className="flex h-full flex-col overflow-y-auto bg-primary-50 py-6 shadow-xl">
                      <div className="px-4 sm:px-6">
                        <div className="flex items-start justify-end">
                          <div className="ml-3 flex h-7 items-center">
                            <button
                              type="button"
                              className="rounded-md bg-primary-50 p-1 text-primary-300 transition duration-100 hover:bg-primary-100 hover:text-white  focus:outline-none"
                              onClick={onCloseDrawer}
                            >
                              <span className="sr-only">Close panel</span>
                              <IoClose size={24} aria-hidden="true" />
                            </button>
                          </div>
                        </div>
                      </div>
                      <div className="relative mt-6 flex-1 px-4 sm:px-6">
                        <div className="flex flex-col items-center">
                          <div className="mb-2">
                            <Avatar user={otherUser} />
                          </div>
                          <p>{title}</p>
                          <p className="text-sm text-gray">{statusText}</p>
                          <div className="my-8 flex gap-10">
                            <div
                              className="flex cursor-pointer flex-col items-center gap-3 hover:opacity-75"
                              onClick={() => setConfirmOpened(true)}
                              aria-hidden="true"
                            >
                              <div
                                className="flex h-10 w-10 items-center justify-center rounded-full bg-danger-400 text-primary-50"
                                onClick={() => {}}
                                aria-hidden="true"
                              >
                                <IoTrash size={20} />
                              </div>
                              <div className="text-xs font-light text-danger-400">
                                Delete
                              </div>
                            </div>
                          </div>
                          <div className="w-full pb-5 pt-5 sm:px-0 sm:pt-0">
                            <dl className="space-y-8 px-4 sm:space-y-6 sm:px-6">
                              {!data.isGroup && (
                                <div>
                                  <dt
                                    className="
                                  text-sm 
                                  font-semibold
                                  text-primary-300
                                  sm:w-40 
                                  sm:flex-shrink-0
                                "
                                  >
                                    Email
                                  </dt>
                                  <dd
                                    className="
                                  mt-1 
                                  text-sm 
                                  sm:col-span-2
                                "
                                  >
                                    {otherUser.email}
                                  </dd>
                                </div>
                              )}
                              {!data.isGroup && (
                                <>
                                  <hr className="text-primary-100" />
                                  <div>
                                    <dt
                                      className="
                                    text-sm 
                                    font-semibold 
                                    text-primary-300 
                                    sm:w-40 
                                    sm:flex-shrink-0
                                  "
                                    >
                                      Joined Ar Luu
                                    </dt>
                                    <dd
                                      className="
                                    mt-1
                                    text-sm 
                                    sm:col-span-2
                                  "
                                    >
                                      <time dateTime={joinedDate}>
                                        Since {joinedDate}
                                      </time>
                                    </dd>
                                  </div>
                                </>
                              )}
                            </dl>
                          </div>
                        </div>
                      </div>
                    </div>
                  </Dialog.Panel>
                </Transition.Child>
              </div>
            </div>
          </div>
        </Dialog>
      </Transition.Root>
    </>
  );
};

export default ProfileDrawer;
